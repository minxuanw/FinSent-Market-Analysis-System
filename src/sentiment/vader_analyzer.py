...
class VaderAnalyzer:
    ...
    def analyze_batch(self, df: pd.DataFrame, text_col: str = None) -> pd.DataFrame:
        """
        Analyze sentiment for a batch of texts.

        Args:
            df: DataFrame with text column
            text_col: Name of the column containing text (auto-detected if None)

        Returns:
            Original DataFrame with sentiment columns appended
        """
        # Auto-detect text column: prefer cleaned, fall back to raw
        if text_col is None:
            text_col = "headline_clean" if "headline_clean" in df.columns else "headline"

        print(f"\n  VADER: Analyzing {len(df)} headlines (using column: {text_col})...")
