...
class DatabaseManager:
    ...
    def insert_sentiment(self, sentiment_df: pd.DataFrame):
        """Insert sentiment scores, skipping duplicates by (article_id, model)."""
        existing = pd.read_sql(text("SELECT article_id, model FROM sentiment_scores"), self.engine)
        if not existing.empty:
            existing_keys = set(zip(existing["article_id"], existing["model"]))
            mask = ~sentiment_df.apply(
                lambda r: (r["article_id"], r["model"]) in existing_keys, axis=1
            )
            new = sentiment_df[mask]
        else:
            new = sentiment_df

        if not new.empty:
            new.to_sql("sentiment_scores", self.engine, if_exists="append", index=False)
            print(f"    Inserted {len(new)} sentiment scores (skipped {len(sentiment_df) - len(new)} existing)")
        else:
            print(f"    No new sentiment scores to insert (all {len(sentiment_df)} already exist)")
